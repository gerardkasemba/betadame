...
// Initialize real-time subscriptions - FIXED: Better balance tracking
useEffect(() => {
  ...
  const fetchDashboardStats = async (agentId: string) => {
    try {
      // Get current agent data for real-time balances
      const { data: currentAgent, error: agentError } = await supabase
        .from('agents')
      ...
      if (agentError) {
        console.error('Error fetching current agent data:', agentError)
      ...
      if (statsData && currentAgent) {
        const withdrawal_commissions = statsData.total_withdrawals * 0.025
        ...
        setStats({
          ...
          available_balance: currentAgent.available_balance, // Use from agents table
          platform_balance: currentAgent.platform_balance,   // Use from agents table
          pending_balance: currentAgent.balance,             // Use from agents table
    ...
    try {
      ...
      const requests: PendingRequest[] = (transactions || []).map(tx => {
        ...
        return {
          ...
          proof_url: tx.qr_code_data,
          user: {
            username: tx.username || 'Utilisateur',
  ...
  const updateRequestCountdowns = () => {
    setPendingRequests(prev => 
      ...
      }).filter(request => request.time_remaining > 0)
    )
  }
