...
export default function LoginForm({ searchParams }: LoginFormProps) {
  ...
  // Check for registration success parameter on component mount
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    clearErrors()
    clearSuccess() // Clear success message when attempting new login
    setIsLoading(true)
    ...
    try {
      ...
      if (result?.error) {
        // Clear any previous success message on error
        setSuccess('')
        ...
        switch (result.error) {
          ...
          case 'Account not activated':
            setError('Votre compte n\'est pas encore activé. Veuillez contacter le support.')
            break
          default:
  ...
  const handleInputChange = (field: 'email' | 'password') => {
    ...
    // Clear success message when user starts typing (optional)
    if (success && !success.includes('Redirection en cours')) {
      setSuccess('')
    }
  }
    ...
    <>
      <div className="flex items-center justify-center bg-gray-50 py-24 px-4 sm:px-6 lg:px-8 min-h-screen">
        ...
        <div className="max-w-md w-full space-y-8 bg-white rounded-2xl shadow-2xl p-8">
          ...
          {/* Success Message - Moved to top for better visibility */}
          {success && (
            <div className="rounded-lg bg-green-50 p-4 border border-green-200 animate-fade-in">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  {success.includes('Redirection en cours') && (
